...
def test_planarFk():
    ...
    test_result = np.cos(1.0) + np.cos(1.0)
    assert test_eval[0] == test_result
    test_result = np.sin(1.0) + np.sin(1.0)
    assert test_eval[1] == test_result
...
def test_planarFk_by_name():
    ...
    fkPlanar = PlanarArmFk(n=2)
    q_np = np.array([1.0, 0.0])
    fkCasadi = fkPlanar.fk(q_ca, "link2", positionOnly=False)
    fkNumpy = fkPlanar.fk(q_np, "link2", positionOnly=False)
    assert fkNumpy[0] == np.cos(1.0) * 1.0 + np.cos(1.0) * 1.0
    assert fkNumpy[1] == np.sin(1.0) * 1.0 + np.sin(1.0) * 1.0
    assert fkNumpy[2] == 1.0
    fkCasadi_fun = ca.Function('test_fun', [q_ca], [fkCasadi])
